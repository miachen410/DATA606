...
```{r}
paste("There are", nrow(cdc), "cases in this data set.")
```
```{r}
paste("There are", ncol(cdc), "variables in this data set.")
```
We can identify each variable's data type by looking at its structure:
```{r}
str(cdc)
```
...
```{r}
# The box plot shows the distribution of different categories of BMI. I chose weight from the data set because it is included in the calcution of finding BMI. I think it should have similar distribution as BMI, and the boxplot seems to match with my guess.
...
```{r}
# There is a linear increasing relationship between weight versus desired weight.
...
```{r}
# "wdiff" is of integer type. If an observation is 0, that means current weight is same as desired weight. If "wdiff" is positive, that means the person is underweight; if negative, then it means the person is overweight.
class(cdc$wdiff)
```
...
```{r}
# I show the distribution of "wdiff" by a boxplot and the summary function. They both show that "wdiff" is centered below 0 (mean = -14.59), it has more spread on the negative values, indicating that there are more overweight population.
boxplot(cdc$wdiff)
summary(cdc$wdiff)
```
...
# Count those within one standard deviation from mean, divided by total number of rows to find the desired proportion
sum(cdc$weight > mean - sd & cdc$weight < mean + sd) / nrow(cdc)
```
