...
> A streak length of one means 1 hit followed by 1 miss. A streak length of zero means there is no hits, only 1 miss.
...
> The distribution is skewed to the right. Kobe's typical streak was zero. His longest streak was four.
...
```
> In my simulation of flipping the unfair coin 100 times, 26 flips came up heads.
...
2)  Describe the distribution of streak lengths. What is the typical streak 
...
```
> The distribution is right skewed. The typical streak length is zero and the longest streak is 5 for this simulated independent shooter with a 45% shooting percentage.

3)  If you were to run the simulation of the independent shooter a second time, 
...
> If I were to run the simulation a second time, I would expect the streak distribution to be somewhat similar because of the law of large numbers, the more times you run the simulation, the closer the shooting rate gets near 45%.

4)   How does Kobe Bryant's distribution of streak lengths compare to the 
...
table(sim_streak)
```
> Kobe Bryant's distribution of streak lengths shows overall better performance than that of the simulated shooter's: 39 zero streaks VS 55 zero streaks; 24 one streak VS 21 one streak; 6 three streaks VS 2 three streaks. However, I don't have sufficient evidence that the hot hand model fits Kobe's shooting patterns because I only ran the simulation once and used it to compare with Kobe's. By the law of large numbers, if I run the simulation many times (say 1000 times), and Kobe still outperforms the simulations, then I would conclude that Kobe's shooting patterns fits the hot hand model.
