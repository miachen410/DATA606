...
> The population distribution is centered around 1500, right skewed, with max outliner 5642.
...
> The distribution of the sample has the same mean as the population's - 1550. It's also right skewed. However there is no outliner as in the population distribution.
...
> I think sample 2 would have a slightly different mean than that of sample 1. As we increase the sample size, the mean should be more closed to the population mean, thus size 1000 would be a more accurate estimate than size 100.
...
> There are 5000 elements in sample_means50, each representing a mean of a sample size 50. The distribution looks very much like a Normal Distribution centered around 1500. I don't expect the distribution to change dramatically even if we instead collected 50,000 sample means.
...
for(i in 1:100){
  small <- sample(area, 50)
  sample_means_small[i] <- mean(small)
}
sample_means_small
```

> There are 100 elements in sample_means_small. Each element represents the mean of a sample size 50.
...
> When the sample size increases, the center remains the same but the spread decreases.
...
> Using this sample, my best point estimate of the population mean is 178,366.    
...
for(i in 1:5000){
  ...
  sample_means50[i] <- mean(sample_50)
}
...
```{r}
mean(sample_means50) #sample mean
```

```{r}
mean(price) #population mean
```
...
for(i in 1:5000){
  sample_150 <- sample(price, 150)
  sample_means150[i] <- mean(sample_150)
}

hist(sample_means150)
```

> Based on this sampling distribution, I'd guess the mean sale price is a little over 180,000.
...
> The sampling distribution with sample sizes 150 has a smaller spread. If we're concerned with making more accurate estimates, we'd prefer a distribution with a small spread.
