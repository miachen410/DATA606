...
> The distribution of men height is unimodal and symmetric, while the distribution of women height is unimodal and skewed.
...
> It's hard to tell. In general, it follows a "bell-shape", with many points above or below the line.
...
> No, not all points fall on the line; some are above, some are below.
...
> It looked similar to the first plot from the simulated data. I believe that the plots provide evidence that the female heights are nearly normal.
...
> The Normal QQ Plot of females' weight doesn't look like any plots from simulated data. It appeared that it might not come from a normal distribution.
...
> What is the probability that a randomly chosen female is shorter than 150cm?

```{r}
pnorm(q = 150, mean = fhgtmean, sd = fhgtsd) #theoretical normal dist.
sum(fdims$hgt < 150) / length(fdims$hgt) #empirical dist.
```

> What is the probability that a randmly chosen female is heavier than 60kg?

```{r}
1 - pnorm(q = 60, mean = fwgtmean, sd = fwgtsd) #theoretical normal dist.
sum(fdims$wgt > 60) / length(fdims$wgt) #empirical dist.
```

> Height has a closer agreement between theoretical and empirical methods.
...
> Plots C and D have a slight stepwise pattern might be caused by rounded age and elbow diameter in the data.
...
>Base on the normal probability plot, female knee diameter is right skewed.
