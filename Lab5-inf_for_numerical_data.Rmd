...
> The cases in this data set are births recorded in the state of North Carolina in 2004.There are 1000 cases in our sample.
...
> Using the `table` function, we see that the cutoff is at age `35`: 
age group `13~34` is considered younger mothers;
age group `35~50` is considered mature mothers.
...
> I tested whether or not if there were significant differences between the average of visits during pregnancy for full term and premie borns. The result shows that the observed difference between the average visits is not zero, in fact full term borns had higher number of visits than premies.   
