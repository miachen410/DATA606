...
> The distribution of the sample is right skewed, with average around 1500 and outliners up to 3820.

2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?

> No I don't expect another sample would be identical. In fact, they might not even be similar because the sample size is small, so it can be varied greatly from one to another.
...
> The sample size must be at least 30 and the distribution must not be strongly skewed.

## Confidence levels

4.  What does "95% confidence" mean? If you're not sure, see Section 4.2.2.

> "95% confidence" means that 95% of the time, the estimate would be within 2 standard deviations from the mean.
...
> Yes, the true average is within my confidence interval.
...
> I would say that nearly 95% of those intervals would capture the true population mean because we can be 95% certain that the estimate would be within the interval.
...
> Looking at the plot, there are 1 out of 50 intervals that didn't capture the true population mean, thus we have 98% of the intervals. It's different from 95% confidence interval because the sample size is not big enough; the bigger the sample size and more trials, the closer our result would be as expected 95% of the time.

-   Pick a confidence level of your choosing, provided it is not 95%. What is 
    the appropriate critical value?
    
> I pick 99% confidence level. Its critical value is 2.58.
...
c(lower_99[1], upper_99[1])
plot_ci(lower_99, upper_99, mean(population))
```


> This time we have 100%, higher than our expected confidence interval of 99%.
